...
public interface FirmApplicationRepository extends JpaRepository<FirmApplication, Long> {
	...
	@Query("SELECT d.officeName FROM FirmApplication d WHERE d.intentAppId.intentAppId = :intentAppId AND d.status =:status")
	String getFirmNameByIntentAppId(@Param("intentAppId")Long intentAppId,  @Param("status")String status);
