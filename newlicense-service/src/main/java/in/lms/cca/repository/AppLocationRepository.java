...
public interface AppLocationRepository extends JpaRepository<AppLocation, Long>{
	...
	@Query("SELECT d FROM AppLocation d WHERE d.intentAppId.intentAppId = :intentAppId AND d.status = :status AND d.created IN " +
		       "(SELECT MAX(d2.created) FROM AppLocation d2 WHERE d2.intentAppId.intentAppId = :intentAppId AND d2.status = :status AND d2.locationName IN ('PR', 'DR', 'Registered Office','OrganizationAddress') GROUP BY d2.locationName)" +
	...
	@Query("SELECT d FROM AppLocation d WHERE d.intentAppId.intentAppId = :intentAppId AND d.created IN " +
		       "(SELECT MAX(d2.created) FROM AppLocation d2 WHERE d2.intentAppId.intentAppId = :intentAppId AND d2.locationName IN ('PR', 'DR', 'Registered Office') GROUP BY d2.locationName) " +
		       "ORDER BY d.created DESC")
	List<AppLocation> findIntentById(@Param("intentAppId") Long intentAppId);
	
	@Query("SELECT d FROM AppLocation d WHERE d.status ='Active'")
