...
public interface FirmApplication2Repository extends JpaRepository<FirmApplication2, Long> {
	...
	@Query("SELECT d FROM FirmApplication2 d WHERE d.intentAppId.intentAppId = :intentAppId AND d.status=:status")
	FirmApplication2 findIntentAppByIdWithStatus(@Param("intentAppId")Long intentAppId,@Param("status") String status);
